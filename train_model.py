...
def generate_one_sampled_build(encoder, decoder, input_str, vocab, device):
    ...
    for t in range(MAX_LEN):
        ...
        if "<trinket>" in generated_tokens or any(t in vocab.dd_data.get('trinkets_all', []) for t in generated_tokens):
            # śledzenie lokalnie w obrębie aktualnego bohatera
    ...
    for batch in loop:
        ...
        with torch.cuda.amp.autocast(enabled=(device.type == 'cuda')):
            ...
            for t in range(1, tgt.size(1)):
                ...
                loss_sum += step_loss
                inp_tok = tgt[:, t] if random.random() < teacher_forcing else logits.argmax(dim=-1).detach()
...
def main():
    ...
    encoder = Encoder(len(dataset.vocab), EMB_SIZE, HID_SIZE, N_LAYERS, DROPOUT).to(DEVICE)
    decoder = Decoder(len(dataset.vocab), EMB_SIZE, HID_SIZE, N_LAYERS, DROPOUT).to(DEVICE)
    ...
    for epoch in range(start_epoch, MAX_EPOCHS + 1):
        ...
        sampled_build = generate_one_sampled_build(encoder, decoder, random_input, dataset.vocab, DEVICE)
